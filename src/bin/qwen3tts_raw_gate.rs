...
fn dump_weights<B: Backend>(talker: &Talker<B>, lane: &str, out: &Path) -> anyhow::Result<()> {
    ...
    for (name, bits, dims) in &weights {
        ...
        wtxt.push_str(&format!("{name} {:x} {} {dims:?}\n", hasher.finalize(), bits.len()));
    }
...
fn dump_forward<B: Backend>(talker: &Talker<B>, lane: &str, out: &Path, prefill: usize, steps: usize, dev: &B::Device) -> anyhow::Result<()> {
    let (hs, am) = run_steps(talker, &synth_ids(prefill, 7), &synth_ids(steps, 99), dev);
    let flat: Vec<f32> = hs.iter().flatten().copied().collect();
    mary::nn::npy::save_npy(&out.join(format!("{lane}_hidden.npy")), &flat, &[hs.len(), talker.hidden])?;
...
fn dump<B: Backend>(talker: &Talker<B>, lane: &str, out: &Path, prefill: usize, steps: usize, dev: &B::Device) -> anyhow::Result<()> {
...
fn compare(dir: &Path) -> anyhow::Result<()> {
    ...
    let read_w = |lane: &str| -> anyhow::Result<Vec<(String, String)>> {
        Ok(std::fs::read_to_string(dir.join(format!("{lane}_weights.txt")))?
            .lines()
            .map(|l| {
    ...
    for s in 0..steps {
        ...
        let mism = r.iter().zip(o).filter(|(x, y)| x.to_bits() != y.to_bits()).count();
        if mism > 0 {
            exact_ok = false;
    ...
    let read_a = |lane: &str| -> anyhow::Result<Vec<usize>> {
        Ok(std::fs::read_to_string(dir.join(format!("{lane}_argmax.txt")))?
            .lines()
    ...
    println!(
        ...
        if exact_ok { "BIT-EXACT ✓" } else { "DIVERGED ✗" },
        if am_ok { format!("{}/{} identical ✓", ar.len(), ao.len()) } else { "DIVERGED ✗".into() }
    );
...
pub fn run() -> anyhow::Result<()> {
    ...
    let lane = arg_str("--lane")
        .ok_or_else(|| anyhow::anyhow!("usage: qwen3tts_raw_gate --lane raw|folded --out <dir> | --compare <dir>"))?;
