...
fn main() {
    let path = std::env::args().nth(1).expect("usage: speak <model.safetensors>");
    ...
    let text_raw = mary::models::f5::tokenizer::Tokenizer::new().encode_tensor::<B>(&format!("{REF} {GEN}"), &device);

    // cond = ref_mel padded with zeros to `duration`
    let gen = duration - ref_len;
    let cond = Tensor::cat(vec![ref_mel, Tensor::<B, 3>::zeros([1, gen, 100], &device)], 1);
    ...
    let vocos = Vocos::<B>::load(&WeightLoader::SingleFile(SingleFileLoader::new(&vweights)), &device);

    // single-forward parity check on the real input (matches prep's v0_cond)
    {
        let v0 = model.forward(y0.clone(), cond.clone(), text.clone(), Tensor::<B, 1>::zeros([1], &device));
        let vd = v0.into_data();
        let sh = vd.shape.to_vec();
        npy::save_npy(&dir.join("v0_cond_burn.npy"), &vd.to_vec::<f32>().unwrap(), &sh).unwrap();
    }

    let cfg = CfmConfig { nfe: 32, sway_coef: -1.0, cfg_strength: 2.0 };
    ...
    let sampled = cfm::integrate(&model, y0, cond, text, &cfg, &device); // [1,dur,100]
    {
        ...
        npy::save_npy(&dir.join("sampled_mel_burn.npy"), &sd.to_vec::<f32>().unwrap(), &sh).unwrap();
    }
    let gen_mel = sampled.slice([0..1, ref_len..duration, 0..100]).swap_dims(1, 2); // [1,100,gen]
    ...
    println!("✓ burn wave {shape:?} → {}", dir.join("burn_wave.npy").display());
}
