...
const IMAGES: &[(&str, &str)] = &[
    (
        "test_images/cartoon_mouse.png",
        "render: a cartoon mouse face",
    ),
    (
        "test_images/portrait.png",
        "render: a generated face/portrait",
    ),
...
fn print_ranking(
    ...
    let mut scored: Vec<(usize, f32)> = img_vecs
        .iter()
        .enumerate()
    ...
    let ranked: Vec<String> = scored
        .iter()
        .map(|(i, s)| format!("{} ({s:+.4})", short(img_paths[*i])))
        .collect();
...
fn main() {
    ...
    let nomic_img: Vec<Vec<f32>> = bytes
        .iter()
        .map(|b| nomic_v.embed_image(b).expect("nomic embed_image"))
        .collect();
    let nomic_q: Vec<Vec<f32>> = QUERIES
        .iter()
        .map(|q| nomic_t.embed_query(q).expect("nomic embed_query"))
        .collect();

    // ---- SigLIP (image+text, 1152-d) ----
    println!("loading SigLIP2 ...");
    let siglip = load_siglip_from_hf(SIGLIP, device.clone()).expect("siglip");
    let siglip_img: Vec<Vec<f32>> = bytes
        .iter()
        .map(|b| siglip.embed_image(b).expect("siglip embed_image"))
        .collect();
    let siglip_q: Vec<Vec<f32>> = QUERIES
        .iter()
        .map(|q| siglip.embed_text(q).expect("siglip embed_text"))
        .collect();
