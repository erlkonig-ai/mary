...
pub fn run() -> anyhow::Result<()> {
    ...
    for (name, handles) in &f16 {
        let (dh, sh) = match handles {
            ...
            LeafHandles::F32(..) => anyhow::bail!("{name}: talker_f16 entity holds an f32 leaf"),
        };
        let bytes: anybytes::Bytes = reader.get(dh).map_err(|e| anyhow::anyhow!("{name}: {e:?}"))?;
        ...
        anyhow::ensure!(shape == f16_shape, "{name}: shape mismatch {shape:?} vs {f16_shape:?}");
        anyhow::ensure!(
    ...
    anyhow::ensure!(misaligned == 0, "{misaligned} f16 leaves misaligned — V3 alignment invariant violated");
    println!(
