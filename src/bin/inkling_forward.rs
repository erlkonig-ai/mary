...
use mary::models::inkling::mtp::{
    Concat as MtpConcat, MtpCache, mtp_block, mtp_block_prefill, mtp_block_step,
};
use mary::models::inkling::pile::Elem;
use mary::models::inkling::source::Weights;
use mary::models::inkling::spectree::{self, TreeSpec};
use mary::models::inkling::stack::embed_and_norm_bf16;
use mary::models::inkling::stepstat;

// The ASSEMBLY — every device-lane component this binary used to define inline.
// It moved to `mary::models::inkling::assembly` verbatim (same statements, same
// order, same allocation sites) so that something OTHER than a binary can build
// a model out of it; see `mary::models::inkling::session`. This binary is now a
// caller of the library it used to be the only copy of.
...
// `Bk`, `Backend`, `BT`, `dev_lane` and `dev_lane_resid` were defined here.
// They are `mary::models::inkling::assembly`'s now, imported above with the rest
// of the assembly, because the components that name them left with it.
...
fn main() -> Result<()> {
    ...
    // Everything one layer carries between generated tokens lives in
    // `assembly::LayerCache` now -- it is the state that makes a SESSION
    // possible at all, so it cannot live inside `main`.
    ...
    loop {
        ...
        for layer in eager_layers {
            ...
            if !layers_dev.contains_key(&p) {
                // The bind itself is `assembly::bind_layer` now. It reads the
                // same tensors in the same order and uploads them at the same
                // point in the pass -- this call site is where the block used to
                // be inline, and the only thing that changed is which file it
                // lives in, because a serving process has to be able to do this
                // without being this binary.
                let b = mary::models::inkling::assembly::bind_layer(
                    &cp,
                    &dev,
                    &fp4_client,
                    fp4_aliases.as_ref(),
                ...
                t_attn_up += b.upload;
                dattn_bytes += b.bytes;
                layers_dev.insert(p.clone(), b.layer);
            }
