//! SigLIP2 so400m embedder parity gate.
//!
//!   cargo run --release --features embed --bin siglip_embed_test
//!
//! 1. Shell out to python (transformers `AutoModel`/`AutoProcessor` on
//!    google/siglip2-so400m-patch14-384) to dump L2-normalized reference vectors
//!    for a test image + 3 texts to /tmp/siglip_ref.json.
//! 2. Embed the same image + texts via `mary::embed::SiglipEmbedder`.
...
const REF_JSON: &str = "/tmp/siglip_ref.json";
const MODEL_ID: &str = "google/siglip2-so400m-patch14-384";
...
with torch.no_grad():
    ...
    tok = proc(text=texts, padding="max_length", max_length=64, truncation=True, return_tensors="pt")
...
fn main() {
    ...
    println!("loading SiglipEmbedder (mary)...");
    let device: WgpuDevice = Default::default();
    let siglip = load_siglip_from_hf(MODEL_ID, device).expect("load siglip");
    assert_eq!(siglip.dim(), SIGLIP_DIM);

    let img_bytes = std::fs::read(IMAGE).expect("read image");
    let rust_image = siglip.embed_image(&img_bytes).expect("embed image");
    let rust_texts: Vec<Vec<f32>> = TEXTS.iter().map(|t| siglip.embed_text(t).expect("embed text")).collect();
