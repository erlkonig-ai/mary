...
fn main() {
    ...
    eprintln!(
        ...
        config.text_config.hidden_size, config.text_config.num_hidden_layers, config.text_config.vocab_size
    );
    ...
    let tokenizer = tokenizers::Tokenizer::from_file(Path::new(&tokenizer_path)).expect("tokenizer");
    let chat = format!("<bos><|turn>user\n{PROMPT}<turn|>\n<|turn>model\n");
    let ids: Vec<i32> = tokenizer.encode(chat.as_str(), false).unwrap()
        .get_ids().iter().map(|&x| x as i32).collect();
    ...
    let l = model.forward_embeds(emb, tokens.clone(), &rope_s, &rope_g, &mut caches, &[], None);
    let [_, sl, vv] = l.dims();
    let last: Vec<f32> = l
    ...
    let am = argmax(&last);
    eprintln!("[parity] mary argmax token id = {am} -> {:?}", tokenizer.decode(&[am as u32], false).unwrap_or_default());
    eprintln!("[parity] vocab = {}", last.len());

    // Write raw little-endian f32[vocab].
    let mut bytes = Vec::with_capacity(last.len() * 4);
    for &x in &last { bytes.extend_from_slice(&x.to_le_bytes()); }
    if let Some(parent) = Path::new(&out).parent() { let _ = std::fs::create_dir_all(parent); }
