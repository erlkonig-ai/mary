...
//! per FIRST TOKEN OF A TURN over the layer range the READY record names, on one
//! box — not per token and not per turn. Turn 0 pays the prefill; every turn
//! after it pays only what is new. That difference is the whole claim.
...
use mary::models::inkling::serve::{Consult, InklingMind, ServeClient};
...
fn serve_client(o: &Options) -> Result<ServeClient> {
    ...
    eprintln!(
        "gate: READY — layers {}..{} of {}, vocab {}, loaded in {:.1}s{}",
        ready.layers[0],
        ready.layers[1],
        ready.stack,
        ready.vocab,
...
fn cmd_drive(o: &Options) -> Result<()> {
    ...
    let mind = InklingMind::new(client, o.tokens, Some(o.system.clone()));
    ...
    *voice_slot.lock().expect("voice slot") = shell.claim_voice();
    ...
    let outcomes = shell.run(
        drive::shell::Extent::Turns(o.turns),
    ...
    // ── the report ──────────────────────────────────────────────────────────
    println!(
        "\n=== drive ran {} turn(s) against a real Session ===",
        outcomes.len()
    );
    ...
    println!("  serving process around the Session calls only, on ONE box over the layer range");
    println!("  the READY record named. Not per token. Not per step.");
...
fn cmd_probe(o: &Options) -> Result<()> {
    ...
    println!("  FRAMING: seconds per FIRST TOKEN OF A TURN, inside the serving process, one box,");
    println!("  over the layer range the READY record named.");
