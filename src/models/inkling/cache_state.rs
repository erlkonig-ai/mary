...
impl TensorState {
    pub fn validate(&self) -> Result<()> {
            ...
            self.row_start <= self.row_end && self.row_end <= self.shape[0],
            "cache tensor live rows exceed its shape"
        );
        let row_bytes = self.shape[1]
            .checked_mul(self.dtype.bytes())
            .context("cache row overflow")?;
        ensure!(
            row_bytes <= MAX_CHUNK_BYTES,
            "cache row exceeds transfer bound"
        );
        self.shape[0]
            .checked_mul(row_bytes)
            .context("cache tensor capacity overflow")?;
        let expected = (self.row_end - self.row_start)
            .checked_mul(row_bytes)
            .context("cache payload length overflow")?;
        let mut actual = 0usize;
        for chunk in &self.chunks {
            ensure!(
                chunk.bytes > 0 && chunk.bytes <= MAX_CHUNK_BYTES && chunk.bytes % row_bytes == 0,
                "cache chunk is empty, oversized, or cuts a row"
            );
            actual = actual
                .checked_add(chunk.bytes)
                .context("cache chunks overflow")?;
        }
        ensure!(
            actual == expected,
            "cache chunks contain {actual} bytes, expected {expected}"
        );
        Ok(())
    }
...
impl KvCacheState {
    pub fn validate(&self, width: usize, max_live: usize, max_capacity: usize) -> Result<()> {
        use super::kvpages::{FP4_ROW_ALIGN, MAX_PAGES, PAGE};
            ...
            self.width == width && width > 0 && self.len <= max_live,
            "KV width or retained length is incompatible"
        );
        ensure!(
            matches!(self.dtype, CacheDType::F32 | CacheDType::Bf16),
            "KV has nonfloat read dtype"
        );
        ensure!(
            !self.fp4 || width % FP4_ROW_ALIGN == 0,
            "packed KV width is misaligned"
        );
        let stored = self
            .head
            .checked_add(self.len)
            .context("KV position overflow")?;
        if self.reserved > 0 {
            ensure!(
                self.pages.len() == 1 && self.epoch > 0 && self.epoch <= self.reserved,
                ...
                self.head < self.epoch
                    && self.fill == stored
                    && self.read >= stored
                    && self.read <= self.reserved,
                "invalid reserved KV head, fill, or read interval"
            );
            ensure!(
                self.pages[0].values.shape[0] == self.reserved,
                "KV reservation disagrees with page"
            );
        } else {
            ensure!(
                self.epoch == 0
                ...
                "invalid growing KV geometry"
            );
        }
        if self.pages.is_empty() {
            ensure!(
                stored == 0 && self.fill == 0 && self.reserved == 0,
                "empty KV has live rows"
            );
            return Ok(());
        }
        ...
        for (i, page) in self.pages.iter().enumerate() {
            ...
            let (cols, dtype) = if self.fp4 {
                (width / 8, CacheDType::I32)
            } else {
                (width, self.dtype)
            };
            ...
            match (&page.scales, self.fp4) {
                (Some(scales), true) => {
                    scales.expect([rows, width / 64], CacheDType::I32, start..end)?
                }
                (None, false) => (),
            ...
            real == stored && capacity <= max_capacity,
            "KV rows exceed stored geometry or admitted capacity"
        );
        ensure!(
            self.reserved > 0 || self.head < self.pages[0].values.row_end,
    ...
    fn tensors(&self) -> impl Iterator<Item = &TensorState> {
        self.pages
            .iter()
            .flat_map(|p| std::iter::once(&p.values).chain(p.scales.iter()))
    }
}
    ...
    fn reservation_rows(&self, context: usize, window: Option<usize>) -> Result<usize> {
        match (window, self.kv_local_rows) {
            (Some(window), Some(rows)) => {
                let needed = window
                    .checked_add(self.prefill_budget.max(self.kv_epoch))
                    .context("local KV append capacity overflow")?;
                let needed = needed
                    .checked_next_multiple_of(self.kv_epoch)
                    .context("local KV reservation overflow")?;
                ensure!(
                    self.kv_prealloc.is_some() && rows == needed,
...
impl SessionCacheState {
    ...
    pub fn validate(&self, expected: &CacheGeometry) -> Result<()> {
            ...
            "unsupported cache image version"
        );
        ensure!(
            &self.geometry == expected,
            "cache model, config, rank, or layout is incompatible"
        );
        ensure!(
            self.identity == expected.identity()?,
            "cache compatibility identity mismatch"
        );
        ensure!(
            expected.kernel > 0
                && expected.hidden > 0
                && expected.context_budget > 0
                && expected.kv_epoch > 0
                && expected.world > 0
                && expected.rank < expected.world
                && !expected.layers.is_empty(),
            "invalid cache geometry"
        );
        ensure!(
            self.position > 0
                && self.position <= expected.context_budget
                && self.position <= u32::MAX as usize,
            "cache image must hold a nonempty representable position"
        );
        ensure!(
            self.layers.len() == expected.layers.len(),
            "cache layer count mismatch"
        );
        ensure!(
            self.last_prediction.is_some_and(|id| id < expected.vocab),
            "invalid cache last prediction"
        );
        ensure!(
            expected.forbidden.iter().all(|&id| id < expected.vocab)
                && expected.forbidden.windows(2).all(|w| w[0] < w[1]),
            "invalid forbidden-token set"
        );
        ensure!(
            !expected
                .forbidden
                .contains(&self.last_prediction.expect("validated above")),
            "cache last prediction is forbidden"
        );
        ensure!(
            self.audio_slot
                .into_iter()
                .chain(self.vision_slot)
                .all(|id| id < expected.vocab),
            "invalid media slot token"
        );
        let hist = expected.kernel - 1;
        let eviction_before = self
            .position
            .saturating_sub(hist.max(expected.sliding_window));
        let mut global_gaps: Option<&[(usize, usize)]> = None;
        for (layer, geometry) in self.layers.iter().zip(&expected.layers) {
            let attn = &layer.attn;
            ensure!(
                attn.k.len == attn.v.len
                    && attn.k.head == attn.v.head
                    && attn.k.fill == attn.v.fill
                    && attn.k.reserved == attn.v.reserved
                    && attn.k.epoch == attn.v.epoch
                    && attn.k.read == attn.v.read,
                "K and V page geometry disagrees"
            );
            ensure!(
                attn.k.pages.iter().map(|p| p.values.shape[0]).eq(attn
                    .v
                    .pages
                    .iter()
                    .map(|p| p.values.shape[0])),
                "K and V page boundaries disagree"
            );
            ...
            for store in [&attn.k, &attn.v] {
                store.validate(geometry.kv_width, live_limit, max_capacity)?;
                    ...
                    expected.kv_local_rows.is_none() || store.reserved > 0,
                    "explicitly preallocated runtime cannot restore a growing KV store"
                );
                if store.reserved > 0 {
                    let context = expected
                        .kv_prealloc
                        .context("cache is reserved but runtime preallocation is off")?;
                    ensure!(
                        store.reserved == expected.reservation_rows(context, geometry.window)?
                            && store.epoch == expected.kv_epoch,
                        "cache reservation does not match runtime"
                    );
                }
            }
            ...
            for &(start, end) in &attn.evicted {
                    ...
                    start >= attn.base
                        && start < end
                        && end <= eviction_before
                        && (skipped == 0 || start > previous_end),
                    "invalid cache eviction intervals"
                );
                skipped = skipped
                    .checked_add(end - start)
                    .context("cache eviction overflow")?;
                previous_end = end;
            }
            let end = attn
                .base
                .checked_add(attn.k.len)
                .and_then(|n| n.checked_add(skipped))
                .context("cache absolute position overflow")?;
            ensure!(
                end == self.position,
                "cache layer does not end at the Session position"
            );
            match geometry.window {
                Some(window) => ensure!(
                    attn.evicted.is_empty()
                        && attn.base == self.position.saturating_sub(window)
                        && attn.k.len == self.position.min(window),
                    "local cache does not hold the full window"
                ),
                None => {
                    ensure!(attn.base == 0, "global cache has a nonzero base");
                    if let Some(gaps) = global_gaps {
                        ensure!(
                            gaps == attn.evicted.as_slice(),
                            "global caches disagree on evicted positions"
                        );
                    } else {
            ...
            attn.k_pre
                .expect([hist, geometry.kv_width], CacheDType::F32, 0..hist)?;
            attn.v_pre
                .expect([hist, geometry.kv_width], CacheDType::F32, 0..hist)?;
            layer
                .attn_sconv
                .expect([hist, expected.hidden], CacheDType::F32, 0..hist)?;
            layer
                .mlp_sconv
                .expect([hist, expected.hidden], CacheDType::F32, 0..hist)?;
        }
...
fn reservation_rows(context: usize, window: Option<usize>, epoch: usize) -> Result<usize> {
    ...
    let rows = match window {
        Some(window) => window
            .checked_add(epoch)
            .context("local cache capacity overflow")?,
        None => context.max(epoch),
    };
    rows.checked_add(epoch - 1)
        .map(|n| n / epoch * epoch)
        .context("cache capacity rounding overflow")
}

fn transfer_rows(client: &ComputeClient<CudaRuntime>, row_bytes: usize) -> Result<usize> {
    ensure!(
        row_bytes > 0 && row_bytes <= MAX_CHUNK_BYTES,
        "invalid cache transfer row width"
    );
    let alignment = (client.properties().memory.alignment as usize).max(1);
    let pitched_row = row_bytes
        .checked_add(alignment - 1)
        .context("cache pitch overflow")?
        / alignment
        * alignment;
...
fn export_handle(
    ...
    sink: &mut BlobSink<'_>,
        ...
        "cache transport requires a little-endian device host"
    );
    ...
    let bytes = shape[0]
        .checked_mul(row_bytes)
        .context("cache shape overflow")?;
    ensure!(
        bytes as u64 <= handle.size_in_used(),
        "cache tensor handle is shorter than its shape"
    );
    ...
    for start in (rows.start..rows.end).step_by(rows_per_chunk) {
        ...
        let raw = client
            .read_one(slice)
            .map_err(|e| anyhow::anyhow!("cache chunk readback: {e:?}"))?;
        ensure!(raw.len() == length, "cache readback length mismatch");
        let handle = sink(raw.as_ref())?;
        ensure!(
            handle == *blake3::hash(raw.as_ref()).as_bytes(),
            "cache sink returned a non-BLAKE3 content handle"
        );
        state.chunks.push(CacheChunk {
            handle,
            bytes: length,
        });
    }
...
fn export_tensor(
    raw: burn_cubecl::tensor::CubeTensor<CudaRuntime>,
    shape: [usize; 2],
    rows: std::ops::Range<usize>,
    sink: &mut BlobSink<'_>,
) -> Result<TensorState> {
    ...
    for start in (rows.start..rows.end).step_by(rows_per_chunk) {
        ...
        let part = burn_cubecl::kernel::into_contiguous(part);
            ...
            part.is_contiguous() && part.dtype == raw.dtype,
            "cache row packing changed layout or dtype unexpectedly"
        );
        let packed = export_handle(
            &part.client,
            part.handle,
            [end - start, shape[1]],
            dtype,
...
pub(crate) fn export_int(
    tensor: &Tensor<Bk, 2, Int>,
    rows: std::ops::Range<usize>,
    sink: &mut BlobSink<'_>,
) -> Result<TensorState> {
...
pub(crate) fn restore_float(
    state: &TensorState,
    client: &ComputeClient<CudaRuntime>,
    dev: &burn::backend::cuda::CudaDevice,
    source: &mut BlobSource<'_>,
) -> Result<Tensor<Bk, 2>> {
    ensure!(
        cfg!(target_endian = "little"),
        "cache transport requires a little-endian device host"
    ...
    state.validate()?;
        ...
        state.dtype != CacheDType::I32,
        "integer plane used as float cache"
    );
    ...
    for chunk in &state.chunks {
        let raw = source(chunk.handle, chunk.bytes)?;
            ...
            *blake3::hash(&raw).as_bytes() == chunk.handle,
            "cache blob content hash mismatch"
        );
        let rows = chunk.bytes / row_bytes;
        let part = seam::tensor_of_dt(
            client.clone(),
            dev.clone(),
        ...
        cubecl::future::block_on(client.sync())
            .map_err(|e| anyhow::anyhow!("cache upload sync: {e:?}"))?;
        row += rows;
    }
    Ok(tensor)
}

pub(crate) fn restore_int(
    state: &TensorState,
    client: &ComputeClient<CudaRuntime>,
    dev: &burn::backend::cuda::CudaDevice,
    source: &mut BlobSource<'_>,
) -> Result<Tensor<Bk, 2, Int>> {
    ensure!(
        cfg!(target_endian = "little"),
        "cache transport requires a little-endian device host"
    );
    ...
    for chunk in &state.chunks {
        ...
        tensor = tensor.slice_assign([row..row + rows, 0..state.shape[1]], part);
        cubecl::future::block_on(client.sync())
            .map_err(|e| anyhow::anyhow!("packed cache upload sync: {e:?}"))?;
        row += rows;
    }
    Ok(tensor)
}
...
mod tests {
    ...
    fn fixture() -> SessionCacheState {
        let geometry = CacheGeometry {
            ...
            flash: true,
            flash_fp4: true,
            head_rms_native: false,
            sink_down_fused: false,
            dense_fake_quant: false,
        ...
        let layers = [None, Some(4)]
            .into_iter()
            .map(|window| {
                let base = window.map_or(0, |w| 12usize.saturating_sub(w));
                LayerCacheState {
                    attn: AttnCacheState {
                        k: store(base, 12 - base),
                        v: store(base, 12 - base),
                        base,
                        evicted: Vec::new(),
        ...
        SessionCacheState {
            ...
            identity: geometry.identity().unwrap(),
            geometry,
            position: 12,
            last_prediction: Some(7),
            audio_slot: None,
            vision_slot: None,
            layers,
        }
    ...
    fn portable_cache_metadata_roundtrip_preserves_full_layer_state() {
        ...
        let restored: SessionCacheState =
    ...
    fn cached_attention_candidate_is_versioned_and_cannot_restore_as_legacy() {
        ...
        assert!(
            serde_json::from_str::<CacheGeometry>(
                &json.replace("packed-batched-v1", "packed-batched-v2")
            )
            .is_err()
        );
    }
    ...
    fn portable_cache_metadata_rejects_malformed_shape_dtype_ranges_and_budget() {
        ...
        for (i, change) in changes.into_iter().enumerate() {
            ...
            change(&mut bad);
            assert!(
                bad.validate(&state.geometry).is_err(),
    ...
    fn portable_cache_explicit_reservation_pins_wide_local_capacity() {
        ...
        assert_eq!(
            ...
            state.geometry.identity().unwrap()
        );
        ...
        state.identity = state.geometry.identity().unwrap();
        assert!(
            state.validate(&state.geometry).is_err(),
            "advertised reservation must exist"
        );
        for (layer, geometry) in state.layers.iter_mut().zip(&state.geometry.layers) {
            let rows = if geometry.window.is_some() {
                1536
            } else {
                1024
            };
