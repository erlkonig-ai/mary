...
//!
//! `depth_gpu_probe dispatch` measures the per-dispatch floor on this Mac at
//! **~6.4 µs** (of which ~4.8 µs is host-side wgpu encode), and this stack's
//! matvecs are SMALL — at q8 a whole layer-step is ~13.6 MB, i.e. ~43 µs of
//! actual streaming. Dispatch count is therefore a first-class cost, not
//! bookkeeping: the layout below is 7 dispatches per layer-step (qkv, attn,
//! o, add+rms, gate‖up+SwiGLU, down, add[+rms]) = 42, plus 3 per step (input
//! build + norm, logit head, argmax/select) and ONE `depformer_in` matvec for
//! the whole frame: `16 · 45 + 1 = 721`. Every fusion in that list exists to
//! delete a floor-priced dispatch, not to save arithmetic.
//!
...
impl DepthGpu {
    /// Load the depformer from `loader`, encode every matvec weight in `fmt`
    /// and upload to the default device. Weight residency: ~1.42 GB (q8),
    /// ~0.75 GB (q4), ~2.67 GB (f16), plus 257 MB of f32 embedding tables.
    ///
    ...
    pub fn load(loader: &WeightLoader, fmt: WeightFmt) -> Self {
        ...
        for i in 0..LAYERS {
            ...
            for (t, step) in steps.iter_mut().enumerate() {
                ...
                step.layers.push(LayerW {
                    qkv: upload(&client, &qkv, 3 * D, D, fmt),
                    o: upload(&client, &o, D, D, fmt),
                    gate_up: upload(&client, &gu, 2 * FH, D, fmt),
                    down: upload(&client, &down, D, FH, fmt),
                });
            }
        }
        ...
        for (t, step) in steps.iter_mut().enumerate() {
            ...
            step.head = upload(&client, &h, CARD, D, fmt);
        }
        let dep_in = upload(&client, &dep_in_all, n * D, cfg::DIM, fmt);
        drop(dep_in_all);
    ...
    /// are meaningless.
    pub fn synthetic(fmt: WeightFmt) -> Self {
        let client = q4::client_for_default_device();
        ...
        let steps: Vec<StepW> = (0..STEPS)
            .map(|_| StepW {
                layers: (0..LAYERS)
                    .map(|_| LayerW {
                        qkv: upload(&client, &fill(3 * D * D), 3 * D, D, fmt),
                        o: upload(&client, &fill(D * D), D, D, fmt),
                        gate_up: upload(&client, &fill(2 * FH * D), 2 * FH, D, fmt),
                        down: upload(&client, &fill(D * FH), D, FH, fmt),
                    })
                    .collect(),
                head: upload(&client, &fill(CARD * D), CARD, D, fmt),
            })
            .collect();
        let norms = (0..LAYERS)
            .map(|_| {
            ...
            .collect();
        let dep_in = upload(&client, &fill(STEPS * D * cfg::DIM), STEPS * D, cfg::DIM, fmt);
